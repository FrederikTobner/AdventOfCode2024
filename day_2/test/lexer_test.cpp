...
TEST_F(LexerTest, HandlesEmptyInput) {
    EXPECT_EQ(aoc::lexer::linebased::tokenize<uint8_t>("", aoc::lexer::rules::handleToken)->size(), 0);
    EXPECT_EQ(aoc::lexer::linebased::tokenize<uint8_t>("\n", aoc::lexer::rules::handleToken)->size(), 0);
    EXPECT_EQ(aoc::lexer::linebased::tokenize<uint8_t>("  \t\n  ", aoc::lexer::rules::handleToken)->size(), 0);
}

TEST_F(LexerTest, ParsesSingleLine) {
    auto result = aoc::lexer::linebased::tokenize<uint8_t>("1 2 3", aoc::lexer::rules::handleToken);
    ASSERT_TRUE(result);
...
TEST_F(LexerTest, ParsesMultipleLines) {
    auto result = aoc::lexer::linebased::tokenize<uint8_t>("1 2 3\n4 5 6\n7 8 9", aoc::lexer::rules::handleToken);
    ASSERT_TRUE(result);
...
TEST_F(LexerTest, HandlesWhitespace) {
    auto result =
        aoc::lexer::linebased::tokenize<uint8_t>("  1  2  3  \n\n  4  5  6  \n  ", aoc::lexer::rules::handleToken);
    ASSERT_TRUE(result);
    EXPECT_TRUE(std::ranges::find(*result, std::vector<uint8_t>{1, 2, 3}) != result->end());
    EXPECT_TRUE(std::ranges::find(*result, std::vector<uint8_t>{4, 5, 6}) != result->end());
}
...
TEST_F(LexerTest, DetectsInvalidInput) {
    EXPECT_FALSE(aoc::lexer::linebased::tokenize<uint8_t>("1 2 a", aoc::lexer::rules::handleToken));
    EXPECT_FALSE(aoc::lexer::linebased::tokenize<uint8_t>("1 2 3\n4 b 6", aoc::lexer::rules::handleToken));
    EXPECT_FALSE(aoc::lexer::linebased::tokenize<uint8_t>("256 1 2", aoc::lexer::rules::handleToken));
}
